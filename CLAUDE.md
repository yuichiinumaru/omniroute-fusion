...
22. **Dashboard IA / design system is mandatory for UI tasks.** Re-homing content without specifying **exactly one hub topbar**, destination peer items, sidebar active-state for deep routes, and anti-phantom chrome tests is incomplete. If the operator request conflicts with `docs/guides/UI.md` or single-topbar law, **stop and ask** before writing tasks. Philosophy: **"Organização tem que ser auto-evidente, ou não é organização."** Full section: root `AGENTS.md` → “Dashboard IA, Design System & Self-Evident Organization”. Precedent: EPIC-19 multi-topbar miss (2026-07-19).
23. **Self-evident UI paths are the target shape** (`/{sidebar-leaf}/{topbar-item}`), not an unbounded `/dashboard/*` dump. Path renames are phased (redirects first); until then, chrome must still be self-evident. Inventory: `docs/reports/audits/2026-07-19-url-ia-self-evident-path-inventory.md`. Do not treat opaque legacy paths as good IA.
