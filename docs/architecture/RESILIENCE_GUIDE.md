...
**Chat soft-failures (Task 0043 / F-04-001):** `handleChatCore` returns `{ success: false, status: 5xx }` without throwing. The chat path must **not** treat that as a circuit-breaker probe success. Use `tryReserveExecution()` (or `canExecute()` for read-only gates) plus explicit `_onSuccess` / `_onFailure` after classifying the soft result via `classifySoftChatBreakerOutcome` (`src/shared/utils/softChatBreakerOutcome.ts` — SSoT). Under **HALF_OPEN**, any soft non-success is a failed probe (`failure` → re-open), including non-provider statuses (429/401), so a burned `tryReserveExecution` slot cannot leave `halfOpenAllowed=0` stuck forever. Outside HALF_OPEN, only provider-level trip codes `[408,500,502,503,504]` record failures; combo defers terminal recording to `recordProviderFailure`. `breaker.execute()` still auto-succeeds on non-throw for non-chat callers.
...
- **18 routing strategies** (priority, weighted, round-robin, context-relay, fill-first, p2c, random, least-used, cost-optimized, reset-aware, reset-window, headroom, strict-random, auto, lkgp, context-optimized, fusion, conditional-fusion) — see [AUTO-COMBO.md](../routing/AUTO-COMBO.md).
