...
| `OMNIROUTE_CLOUD_SYNC_SECRET`           | _(empty)_                                                       | `src/lib/cloudSync.ts`                      | Shared secret used to verify the HMAC-SHA256 signature of Cloud Sync responses **and** sign outbound requests. **Required** for cloud sync (fail-closed when unset unless `OMNIROUTE_CLOUD_SYNC_INSECURE=1`).                                                                                                                                                      |
| `OMNIROUTE_CLOUD_SYNC_SECRETS`          | `false`                                                         | `src/lib/cloudSync.ts`                      | Set to `true` to allow Cloud Sync to overwrite local credentials **and** to upload OAuth tokens / API keys outbound. Default is `false` (metadata-only).                                                                                                                                          |
| `OMNIROUTE_CLOUD_SYNC_INSECURE`         | _(unset)_                                                       | `src/lib/cloudSync.ts`                      | Set to `1` only for local/dev legacy mode: allows cloud sync without `OMNIROUTE_CLOUD_SYNC_SECRET`. **Never enable in production.**                                                                                                                                                              |
