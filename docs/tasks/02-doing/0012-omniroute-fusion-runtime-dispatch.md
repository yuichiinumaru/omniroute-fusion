...
> **Status**: `[ ]` Returned to doing (reaudit REJECT 88 — nested comboChatBase / base-option forwarding)
> **Priority**: 🟡 P1
> **Type**: `feature`
...
- **Reviewer**: reviewers (OmniRoute Architect adversarial re-audit)
- **Data da review**: 2026-07-16
- **Veredito**: REJECTED_TO_DOING (S=88 < 90)
- **Score (path to 100)**: 88/100
- **Notas**: Nested combo-ref option forwarding (F1) still unfixed on live FS; no comboChatBase; no regression test. Returned to doing.
...
- **Lane outcome**: returned to doing (`02-doing/`)
- **Task reference**: Task 0012 (`omniroute-fusion-runtime-dispatch`)

#### Current Open Blockers

- `RESOLVED on FS (cross-task fixer wave 0013)` F1: `comboChatBase` on V2 + nested spread in `dispatchFusionUnit`
- `RESOLVED on FS` F4: unit `V2: comboChatBase settings/signal/acl forward into nested handleComboChat`
- `RESOLVED on FS` F5/F6 impact path closed by same base-option thread
- `PERSISTENT` F2: dedicated CHANGELOG Task 0012 entry still optional (path-to-100 residual close mentions 0012/0013 F1)
- `PERSISTENT` F3 (Improvement/residual): single-survivor re-dispatch double-invokes unit

#### Path-to-100 Summary

- F1/F4 production + test landed via path-to-100 FIXER (parent reviewers) while closing Task 0013 — re-verify live FS then return to `03-review/`
- Optional: dedicated Task 0012 CHANGELOG bullet; F3 single-survivor double-invoke polish
...
Task remains in `02-doing/` for 0012 owner re-verify + optional F2/F3 polish; do not treat as incomplete production for F1.