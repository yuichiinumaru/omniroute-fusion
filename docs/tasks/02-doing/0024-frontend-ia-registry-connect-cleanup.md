...
> **Status**: `[ ]` Returned to doing — adversarial re-audit 86/100 REJECTED_TO_DOING (2026-07-16; dual catalog via Operations hub 0059)
> **Priority**: 🔴 P0
> **Type**: `feature`
...
- **Date**: 2026-07-16 (adversarial re-audit)
- **Reviewer profile**: `reviewers` (agentID=reviewers)
- **Score**: `86/100`
- **Verdict**: `REJECTED_TO_DOING`
- **Full report**: `docs/reports/reviews/2026-07-16-task-0024-frontend-ia-registry-connect-cleanup-reaudit.md`
- **Lane outcome**: **returned to `docs/tasks/02-doing/`** (S < 90)
- **Task reference**: Task 0024 (`frontend-ia-registry-connect-cleanup`); resolve path under `docs/tasks/`

#### Current Open Blockers

- `REGRESSION` (Medium): Operations hub (Task 0059) dual-lists `/dashboard/api-endpoints` **and** `/dashboard/endpoint?tab=catalog` — reintroduces dual catalog homes outside primary sidebar
- `NEW` (Medium): `connect-exposure-sidebar` tests only guard primary peers — phantom green while hub dual homes exist; 0059 tests **require** both hrefs
- `PERSISTENT` (Low): Playwright e2e smoke still unevidenced

#### Path-to-100 Summary

1. Remove Operations hub card for legacy `/dashboard/api-endpoints` (keep single catalog SSoT card)
2. Update `operations-hub-discoverability-0059.test.ts` required hrefs accordingly
3. Strengthen connect-exposure / shared IA guard: hub must not dual-home catalog
4. Residual e2e optional; **do not** undo S5 endpoint tabs / protocol redirects / hideables / PROVENANCE

#### Regression Guards

- Default chrome: no `api-endpoints` leaf; no triple MCP/A2A/API Endpoints peers; `api-manager` stays keys-separate
- `/dashboard/api-endpoints` remains **redirect-only**, not a discovery peer (Operations hub, palette, etc.)
- Flat primary may nest exposures under hubs; hideable ids for `api-endpoints`, `endpoints`, `mcp`, `a2a` retained
- `/dashboard/api-endpoints` → `endpoint?tab=catalog`; `endpoint?tab=mcp|a2a` → protocol homes
- Do not re-embed MCP/A2A dashboards as Endpoint peer tabs
