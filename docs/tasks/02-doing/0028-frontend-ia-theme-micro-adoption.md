...
> **Status**: `[~]` Return to doing (adversarial reaudit 2026-07-16 — S=88; glow regression tests red + brand SSoT superseded by 0052/0053)
> **Priority**: 🟡 P1
> **Type**: `feature`
...
- **Full report**: `docs/reports/reviews/2026-07-16-task-0028-frontend-ia-theme-micro-adoption-reaudit.md`
- **Lane outcome**: **returned to `02-doing/`** (S < 90)
- **Task reference**: Task 0028 (`frontend-ia-theme-micro-adoption`)

#### Current Open Blockers

- `REGRESSION` High: `status-vocabulary.test.ts` + `badge-status.test.tsx` fail (glow asserts `shadow-` / inline `var(`; live uses `status-glow-*` CSS classes)
- `SUPERSEDED` Medium: original coral-default + optional cyan Appearance exit intentionally flipped by Tasks 0052/0053 — update docs/guards, do not re-litigate brand
- `NEW` Low: `statusVocabulary.ts` header still claims coral SSoT

#### Path-to-100 Summary

- Fix glow regression tests to assert `status-glow-*` + `globals.css` rules
- Refresh module header / 0028 brand narrative for coreCyan dark-only successor truth
- Optional ModelPill amber polish

#### Regression Guards (updated 2026-07-16)

- Soft glow must remain limited to health/breaker surfaces
- Glow utilities resolve via `status-glow-*` classes backed by `--status-glow-*` in `globals.css`
- StatCard `accent` default **`none`**
- No Orbitron / scanlines / Prism / `visual-reference` production imports under `src/`
- Badge `warning` chroma stays **amber**
- Brand default is now **coreCyan dark-only** (0052/0053 supersedes coral optional preset guard)
