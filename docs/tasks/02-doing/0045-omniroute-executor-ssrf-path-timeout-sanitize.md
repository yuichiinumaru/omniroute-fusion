...
> **Status**: `[~]` Path-to-100 after adversarial re-audit (S=88)
> **Priority**: 🟠 P1
> **Type**: `remediation`
...
- **Date**: 2026-07-16
- **Reviewer profile**: `reviewers` (adversarial re-audit)
- **Score**: `88/100`
...
- `NEW` N6 (Medium): `isSafeChatPath` accepts `//evil.com`, `%2e%2e`, `\` — denylist incomplete vs path-injection claim
- `PERSISTENT` N2 (Medium / Hard Rule #12): chatgpt-web `errorResponse` still raw `err.message` on HTTP catch
- `PERSISTENT` N1 (Medium): muse-spark / claude-web / grok-web still full-request `AbortSignal.timeout(FETCH_TIMEOUT_MS)`
- `NEW` N7 (Medium sibling): Qoder `customApiBase`/`resourceUrl` open base with Bearer (no host allowlist)
- `PERSISTENT` N4 (Low): dual `assertSafePathSegment` (open-sse weak vs `src/shared/network` allowlist)

#### Path-to-100 Summary

1. Harden `isSafeChatPath` (reject `//`, `\`, `%` / encoded traversal); unit cases
2. Sanitize chatgpt-web `errorResponse` callers via `sanitizeErrorMessage` / `buildErrorBody`
3. Migrate muse-spark (+ ideally claude/grok web) to `fetchWithStartTimeout`
4. Document or guard Qoder base URL (N7)
5. Optional: Qwen redirect re-validation; collapse path-segment SSoT

#### Regression Guards (must stay green)

- DefaultExecutor/Base `resolveSafeChatPath` production wire
...
- `2026-07-11` — `91/100` — `docs/reports/reviews/2026-07-11-task-0045-executor-ssrf-review.md`
  - **Carried forward**: N1 timeout residuals, N2 chatgpt #12, N3 devin-cli, N4 dual path helper, N5 thin tests
  - **Resolved since**: none (no product path-to-100 work before reaudit)
