...
- [ ] `.changelog/` entry is created through manage-changelog and rebuilt. (parent orchestrator duty)
...
- **Reviewer/verdict/score**: Pending independent review
- **Notes**: Implementation complete; expert polish verified all exit conditions fresh (2026-08-05). Semantic audit confirmed family membership aligns with `ANTIGRAVITY_PUBLIC_MODELS` and `AGY_PUBLIC_MODELS` canonical sources. Both `antigravity` and `agy` providers route through `parseAntigravity` → `groupAntigravityQuotas`. Coarse `getAntigravityQuotaFamily` preserved for `accountFallback` lockouts. Hidden-model filtering handles family bars via `modelKeys`. Minimum-remaining aggregation, credits/unknown retention, and reset/stale metadata all verified. Ready for review.
