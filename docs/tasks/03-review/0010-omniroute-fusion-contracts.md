...
- **Notas**: Contracts complete and production-wired (not dead types). See Review Ledger + full report.
...
- **Task reference**: Task 0010 (`omniroute-fusion-contracts`); path `docs/tasks/03-review/0010-omniroute-fusion-contracts.md`

#### Current Open Blockers

- none blocking (score ≥ 90)
- `NEW` Improvement: CHANGELOG draft not yet published (parent wave closeout)
- `NEW` Improvement deferred: `validateComboDAG` does not walk top-level `judge`/`acting` combo-refs (runtime cycle/depth still safe)

#### Path-to-100 Summary

- Publish drafted CHANGELOG entry at parent wave closeout
- Optional: schema → `resolveFusionUnits` round-trip unit
- Defer DAG judge/acting walk to Task 0018 hardening

#### Regression Guards

- Keep D8 superRefine rejecting `fallbackStrategy` ∈ {fusion, conditional-fusion} (case-insensitive)
- Keep `triggers.mode` enum always | tool-call | text-match + `textPatterns`
- Keep top-level `judge` as `comboModelEntry` (no `role:"judge"` on steps)
- Keep `ResolvedFusionUnit` shape per epic §5.3 (model | combo-ref + optional label)
