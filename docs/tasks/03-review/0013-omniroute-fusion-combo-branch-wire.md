...
- **Notas**: Production entry PROVEN (chat → handleComboChat → dispatchFusionStrategy → handleFusionChatV2). Not dead code. Residual shared with 0012 nested base options.
...
- **Task reference**: Task 0013 (`omniroute-fusion-combo-branch-wire`)
- **Production entry**: PROVEN — integration matrix + unit combo-ref panel

#### Current Open Blockers

- `NEW` F1 (Debt, shared w/ 0012): cannot forward parent combo base options until V2 API accepts them
- `NEW` F2 (Improvement): optional spy that fusion path passes typed panels + `handleComboChat` ref

#### Path-to-100 Summary

- After 0012 adds `comboChatBase`, thread `settings` / `isModelAvailable` / `relayOptions` / `signal` / `apiKeyAllowedConnections` from `handleComboChat` into `dispatchFusionStrategy`
- Keep conditional-fusion trigger/fallback and combo-ref non-drop regression guards

### Previous Reports

- none (initial review)
- Related: `docs/reports/reviews/2026-07-10-task-0012-omniroute-fusion-runtime-dispatch-review.md`
