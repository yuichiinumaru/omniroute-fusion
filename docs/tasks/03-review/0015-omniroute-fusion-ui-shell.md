...
- **Notas**: Shell wiring solid (sidebar, list filter, create/delete, API). Stay in 03-review. Full report linked in Review Ledger.
...
- **Task reference**: Task 0015 (`omniroute-fusion-ui-shell`)

#### Current Open Blockers

- none blocking merge of shell contract
- `NEW` F1 (Improvement): Card click-to-edit keyboard affordance
- `NEW` F2 (Improvement): list chrome i18n (Task 0017 scope)
- `NEW` F3 (Debt): unit test for fusion list filter helper

#### Path-to-100 Summary

- Add unit test for fusion strategy filter (+ hidden exclusion)
- Optional: keyboard-accessible card navigation or drop whole-card onClick
- Optional: next-intl for list chrome with Task 0017
