...
- **Notas**: Final re-review closed residuals. Report: `docs/reports/reviews/2026-07-18-task-0011-omniroute-fusion-resolve-units-final-review.md`. Stay in `03-review/`.
...
- CHANGELOG + EMPTY_FUSION_JUDGE + invalid-judge fallthrough closed

#### Patches Applied This Final Review

CHANGELOG Task 0011; verified EMPTY_FUSION_JUDGE + fallthrough units
...
- `docs/reports/reviews/2026-07-18-task-0011-omniroute-fusion-resolve-units-final-review.md` — 100/100 APPROVE (**latest**)
- `docs/reports/reviews/2026-07-16-task-0011-omniroute-fusion-resolve-units-reaudit.md` — prior reaudit
...
### Residuals closed
- **F1 CHANGELOG**: published Unreleased Added bullet for Task 0011.
- **F2 EMPTY_FUSION_JUDGE**: exported sentinel from `open-sse/services/fusion.ts`; `resolveJudgeUnit` returns it; unit asserts identity.
- **Optional invalid judge fallthrough**: unit covers invalid `data.judge` → `judgeModel` → first panel.

### Files changed
- `open-sse/services/fusion.ts` (`EMPTY_FUSION_JUDGE`)
- `tests/unit/fusion-units-resolve.test.ts` (+2 tests)
- `CHANGELOG.md`

### Commands
```bash
node --import tsx/esm --test tests/unit/fusion-units-resolve.test.ts  # 16/16 pass
```
