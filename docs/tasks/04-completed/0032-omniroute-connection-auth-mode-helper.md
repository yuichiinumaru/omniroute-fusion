...
- [x] **Read existing code**: `src/lib/tokenHealthCheck.ts` (helper + #5326 branch), `src/lib/db/providers.ts` (authType filter + `createProviderConnection` default `authType || "oauth"` ~L369), `open-sse/services/tokenRefresh.ts` (`supportsTokenRefresh`), `src/app/api/providers/[id]/refresh/route.ts`, `src/app/api/providers/[id]/test/route.ts` (oauth vs qoder apikey branches), `tests/unit/token-health-no-refresh-token-expired-5326.test.ts`, import graph for `src/shared` ↔ `open-sse` cycles
- [x] **Choose module location** that avoids circular deps; if shared lives under `src/shared/utils/`, keep open-sse call sites on Task 0035 unless path is safe for open-sse
- [x] **Implement pure helpers** with JSDoc: dual-mode rules, blank authType + apiKey foot-gun
- [x] **Wire `tokenHealthCheck.ts`** to import shared helper; re-export for back-compat if tests import from health module
- [x] **Write unit tests** for normalize + connectionUsesOAuthRefresh matrix
- [x] **Refactoring pass**: no behavior change beyond single source of truth; delete duplicated branch only after tests green
- [x] **Verification**: typecheck + lint + named unit tests
...
- [x] **Doc Accuracy**: Function/path names grepped before documenting
- [x] **Zod Validation**: N/A unless new API surface (none expected)
- [x] **Security**: No secrets; no credential logging
- [x] **Error Sanitization**: N/A (pure helpers)
- [x] **No Raw SQL**: N/A
- [x] **Archive Protocol**: Move/extract, do not delete history of behavior
...
- **Resultado dos testes**: PASS — pure helper 15 + #5326 7 (path-to-100 2026-07-18 epic suite green)
...
- **Report**: `docs/reports/reviews/2026-07-18-task-0032-connection-auth-mode-helper-final-review.md`
- **Notas**:
...
- **Full report**: `docs/reports/reviews/2026-07-18-task-0032-connection-auth-mode-helper-final-review.md`
- **Lane outcome**: remains in review (`03-review/`)
- **Task reference**: Task (omniroute-connection-auth-mode-helper)
- **Patches applied this review**: plain-object gate (reject arrays); SAFETY PSD; Details hygiene; CHANGELOG polish
...
- `2026-07-18` — `100/100` — `docs/reports/reviews/2026-07-18-task-0032-connection-auth-mode-helper-final-review.md`
...
- `93/100` prior reaudit — `docs/reports/reviews/2026-07-16-task-0032-connection-auth-mode-helper-reaudit.md` (score UNTRUSTED for history only)
