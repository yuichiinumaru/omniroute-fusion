...
- **Full report**: `docs/reports/reviews/2026-07-16-task-0039-provider-auth-status-limits-i18n-final-review.md`
- **Lane outcome**: remains in `03-review` (S=100; not demoted; not auto-promoted)
- **Task reference**: Task 0039 (`omniroute-provider-auth-status-limits-i18n`)
...
- ✅ **N1**: 401 row surfaces translated detail + CTA (`— ${cta}`)
- ✅ **N2**: unit-test `translateUsageOrFallback` hit / miss / `__MISSING__` strip
- ✅ **N3**: formatter packs true expiry under `expired` id (shared with 0037)
- ✅ **N4**: unknown/blank auth no longer invents OAuth CTA (0037 final-gate helper)
- ✅ **N5** (epic): ProviderCard/ListRow consume `providers.connectionStatus` keys (0038)

#### Path-to-100 Fix (2026-07-18 final-gate)

- **Reviewer**: `reviewers` — re-verified limits wire + i18n; N4 closed via 0037 helper
- **Tests**: limits + combined suite green (35/35)
- **Lane**: remain `03-review/` at 100

#### Regression Guards

- ProviderLimits must not reintroduce hard-coded OAuth re-auth suffix
- 401 path must keep `formatQuotaAuthErrorMessage` + `tr(copy.keys.detail, fallback)` + CTA
- EN `usage.connectionStatus` + `providers.connectionStatus` full id×field matrix
- `__MISSING__:` must strip to EN (never show sentinel to operators)

### Previous Reports

- `2026-07-16 final` — `100/100` — `docs/reports/reviews/2026-07-16-task-0039-provider-auth-status-limits-i18n-final-review.md`
  - **Resolved**: N1–N5; score 100
  - **Regression guard**: limits copy suite + EN key presence test
- `2026-07-16 reaudit` — `94/100` — `docs/reports/reviews/2026-07-16-task-0039-provider-auth-status-limits-i18n-reaudit.md`
  - **Carried forward then fixed**: N1–N3/N5; N4 via 0037
  - **Resolved since**: path-to-100 + final-gate
  - **Regression guard**: limits copy suite + EN key presence test
- `2026-07-11` — `94/100` — `docs/reports/reviews/2026-07-11-task-0039-provider-auth-status-limits-i18n-review.md`
  - **Carried forward**: N1 detail-only UI; N2 strip untested; N3 expired id; N4 unknown→oauth
  - **Resolved since**: path-to-100 + 0037 final-gate
  - **Regression guard**: limits copy suite + EN key presence test
