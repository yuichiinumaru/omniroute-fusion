...
async function buildNonStreamingResponse(
  ...
  // Guard: if still empty after all processing (SSE contents + image polls +
  // pointer resolution), return a clear error instead of ~0 tokens. This is
  // a catch-all for edge cases like upstream error pages that look like SSE
  // or malformed SSE without a recognizable error event.
  if (!fullAnswer.trim()) {
  ...
  async execute({
    ...
    try {
      response = await tlsFetchChatGpt(CONV_URL, {
        ...
        // local buffering (temp-file tailing vs full-body in memory), NOT the
        // HTTP protocol. ChatGPT always returns SSE regardless of what the
        // caller requested. Using stream:false buffers the entire response and
        // can timeout on long generations; stream:true tails a temp file and
        // surfaces a ReadableStream as bytes arrive.
        stream: true,
      });
    } catch (err) {
    ...
    if (response.status >= 400) {
      ...
      // Log upstream body AND rate-limit headers on4xx/5xx. The TLS client
      // preserves real status+headers for non-SSE responses (JSON errors,
      // 429, Cloudflare challenges), giving us the diagnostic signal we
      // need — unlike the SSE path where status is always forced to 200.
      const rlHeaders = [
        ...
        .join(" ");
      log?.warn?.("CGPT-WEB", `conv ${status}: ${rlHeaders}`);
