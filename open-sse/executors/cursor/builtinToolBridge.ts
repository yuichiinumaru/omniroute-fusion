// Canonical Cursor native-tool bridge. Maps Cursor's native shell/read
// events and the native TodoWrite completion envelope onto the external
// tool-call/session contract used by the OpenAI-facing pipeline. Fail-closed
// on malformed or unsupported events — the bridge never invents a tool
// result. Symmetric with the upstream decolua/9router#1380 reference.
...
import { flattenMessages } from "../../utils/cursorAgentProtobuf.ts";
...
/**
 * Infer the caller's Cursor client platform from system-message metadata.
 * Returns undefined when no system message carries a unique platform signal
 * (mixed signals, ambiguous mention, or absent). Used by `pty_spawn` bridges
 * to pick PowerShell vs /bin/sh.
 */
...
type CursorShellExecEvent = Extract<
  ExecServerEvent,
  { kind: "exec_shell" | "exec_shell_stream" | "exec_bg_shell" }
>;

function directShellBridge(
  event: CursorShellExecEvent,
