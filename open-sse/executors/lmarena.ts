...
export class LMArenaExecutor extends BaseExecutor {
  ...
  buildUrl(_model?: string, _stream?: boolean, _urlIndex?: number, _credentials?: unknown): string {
    return LMARENA_STREAM_URL;
  }

  buildHeaders(
    modelOrCredentials?: unknown,
    credentialsOrStream?: unknown,
    _bodyOrClientHeaders?: unknown
  ): Record<string, string> {
    const credentials =
      typeof modelOrCredentials === "string"
        ? credentialsOrStream
        : modelOrCredentials;
