...
export type RequestLogsPanelProps = {
  /** Pre-select a call log row (from hub `?id=` / legacy `?request=`). */
  initialSelectedId?: string | null;
};

/**
 * Request-log stream chrome + RequestLoggerV2.
 * Composed by the Observe hub (`/dashboard/activity?source=request`).
 */
export default function RequestLogsPanel({ initialSelectedId = null }: RequestLogsPanelProps) {
  ...
  const requestLoggerRef = useRef(null);
