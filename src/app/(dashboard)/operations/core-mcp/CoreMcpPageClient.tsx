...
import { OPERATIONS_TOPBAR_LABELS } from "@/shared/constants/epic20Operations";
import { copyToClipboard } from "@/shared/utils/clipboard";
import McpDashboardPage from "@/app/(dashboard)/dashboard/endpoint/components/MCPDashboard";

type ServiceStatus = { online: boolean; loading: boolean };
type McpTransport = "stdio" | "sse" | "streamable-http";

const CORE_MCP_LABEL = OPERATIONS_TOPBAR_LABELS["core-mcp"];
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        ...
        aria-label={enabled ? t("disableLabel", { label }) : t("enableLabel", { label })}
        onClick={onToggle}
        disabled={toggling}
...
function TransportSelector({
  ...
  // Technical API paths stay `/api/mcp/*` (do not rename to core-mcp).
...
/**
 * EPIC-20 T20-D / Task 0089 — CoreMCP control-plane body.
 * Content-only: Operations topbar is layout-owned (0087). No Endpoint tab chrome.
 */
export default function CoreMcpPageClient() {
  const t = useTranslations("mcpDashboard");
  ...
  return (
    <div className="flex flex-col gap-6" data-testid="coremcp-page" data-coremcp-page="">
      {/* Operational controls at top — enable toggle always reachable */}
      <Card>
      ...
      {mcpEnabled && (
        <TransportSelector
          value={mcpTransport}
          onChange={(transport) => void changeTransport(transport)}
      ...
      {/* Prose explainers: bottom, default collapsed (EPIC-20 / 0089) */}
      <div data-coremcp-explainer="" data-testid="coremcp-explainer">
        <Collapsible
          title={t("howToTitle")}
