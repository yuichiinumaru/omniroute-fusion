 ...
 *
 * The pattern is intentionally narrow: three base64url segments separated
 * by `.`, prefix `eyJ` (the canonical JWT JOSE header). Anything else is
 * passed through unchanged so the operator still sees useful upstream
 * context (e.g. "client_id is required").
 *
 * This is in addition to the sanitizeErrorMessage() / buildErrorBody()
 * helpers used by the OAuth route and the executors — it specifically
 * guards the throw-sites inside the OAuth provider modules where the
 * upstream message is the only signal we have to surface to the caller.
 */
...
export function redactGrokBuildSecrets(value: string): string {
  ...
  // Preserve the scheme/key prefix for diagnostics (e.g. "Bearer [REDACTED]",
  // "token=[REDACTED]") while removing the secret value itself.
  ...
  out = out.replace(GROK_BUILD_TOKEN_COOKIE_SHAPE, (match) => {
    ...
    // When the prefix capture fails (edge shape), fall back to full placeholder
...
async function requestDeviceCode(_config?: unknown) {
  ...
  if (!response.ok) {
    ...
    // The upstream `error_description` is the only signal we have, but it can
    // echo authorization codes / tokens (xAI has done this historically),
    // so redact any JWT-shaped substring before re-throwing. Non-token
    // messages (e.g. "client_id is required") are forwarded verbatim.
    throw new Error(
      description !== null
        ? redactGrokBuildSecrets(description)
...
async function pollToken(_config: unknown, deviceCode: string) {
  ...
  // F4: the polling error body is the ONLY user-visible signal for
  // expired/denied/authorization_pending/slow_down, and xAI has echoed
  // tokens inside error_description — redact any JWT-shaped substring
  // before it leaves the provider boundary, while keeping the
  // actionable prefix ("The device code has expired", etc.).
  if (!response.ok || parsed.error) {
    const desc = typeof parsed.error_description === "string" ? parsed.error_description : null;
    if (desc) {
      // parseOAuthResponse returns a plain object with mutable string values —
      // replace only the one field we own.
      parsed.error_description = redactGrokBuildSecrets(desc);
    }
    const msg = typeof parsed.message === "string" ? parsed.message : null;
    if (msg) parsed.message = redactGrokBuildSecrets(msg);
  }
