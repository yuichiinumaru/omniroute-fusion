...
import { BX_UMIDTOKEN_FALLBACK } from "@omniroute/open-sse/services/qwenTlsClient.ts";
...
export async function validateQwenWebProvider({ apiKey }: any) {
  ...
  try {
    ...
    const { tlsFetchQwen, TlsClientUnavailableError } =
      await import("@omniroute/open-sse/services/qwenTlsClient.ts");

    let resp;
    try {
      resp = await tlsFetchQwen("https://chat.qwen.ai/api/v2/user", {
        method: "GET",
        headers,
        timeoutMs: 15_000,
      });
      ...
      if (err instanceof TlsClientUnavailableError) {
        return {
          valid: false,
          error: `TLS impersonation client unavailable: ${err.message} (qwen-web requires this — without it, Alibaba's WAF blocks every request)`,
        };
      }
      throw err;
    }
...
export async function validateLMArenaProvider({ apiKey }: any) {
  try {
    ...
    if (!raw) {
      return {
        valid: false,
        error:
          "Missing LMArena session cookie — paste the full Cookie header from lmarena.ai (DevTools → Network → request → Cookie)",
      };
    }

    // Reconstruct the LMArena cookie from the pasted blob (handles Supabase SSR
    // chunked cookies: arena-auth-prod-v1.0, .1, …).
    const { reconstructLMArenaCookie } = await import(
      "@omniroute/open-sse/executors/lmarena.ts"
    );
    const cookieHeader = reconstructLMArenaCookie(raw);
    if (!cookieHeader) {
      return {
        valid: false,
        error:
          "Could not extract a usable cookie from the pasted value — paste the full Cookie header from lmarena.ai",
      };
    }
    ...
    if (res.status === 401 || res.status === 403) {
      return {
        valid: false,
        error:
          "LMArena session is invalid or expired — re-login at https://lmarena.ai and paste a fresh Cookie header",
      };
    }

    // Any non-auth response (200, 400, 422, 429) means the cookie was accepted.
    return { valid: true, error: null };
  } catch (error: any) {
    return toValidationErrorResult(error);
  }
