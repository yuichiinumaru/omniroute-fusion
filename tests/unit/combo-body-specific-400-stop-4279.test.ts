...
test("#5249 combo continues after a model-specific 400 invalid model error", async () => {
  const modelsCalled: string[] = [];
  const handleSingleModel = async (_body: unknown, modelStr: string) => {
    modelsCalled.push(modelStr);
  ...
  const result = await handleComboChat({
    body: { model: "test", messages: [{ role: "user", content: "hi" }] },
    combo: makeCombo(["codex/missing-model", "codex/working-model"]),
    handleSingleModel,
    log,
    settings: {},
    allCombos: [],
  ...
  assert.deepEqual(modelsCalled, ["codex/missing-model", "codex/working-model"]);
