...
const { isContextOverflow400, isModelAccess400, isParamValidation400 } = await import(
...
test("#5249 combo guard: model-specific 400s are not body-specific stop conditions", () => {
  assert.equal(isModelAccess400("Invalid model: this account cannot access the requested model."), true);
  assert.equal(
    isModelAccess400("unauthorized-ish blob", { code: "model_not_found" }),
    true
  );
  assert.equal(isModelAccess400("Invalid message format: the request body is malformed."), false);
});
