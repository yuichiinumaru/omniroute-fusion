...
test("isCursorAgentCliVersionId validates the dated-build shape", () => {
  assert.equal(isCursorAgentCliVersionId("2026.07.08-0c04a8a"), true);
  assert.equal(isCursorAgentCliVersionId("2026.01.01-aaaaaaa"), true);
  // IDE semver form is NOT a CLI build id
  assert.equal(isCursorAgentCliVersionId("3.9"), false);
  assert.equal(isCursorAgentCliVersionId(""), false);
  assert.equal(isCursorAgentCliVersionId("not-a-version"), false);
});
...
// ─── getCursorAgentCliVersion: env > FS > pin ────────────────────────────
...
// ─── Determinism + safe fallback ─────────────────────────────────────────

test("getCursorAgentCliVersion returns the pin deterministically when nothing is set", () => {
  // We cannot guarantee HOME is empty on the test runner, so we isolate HOME
  // and ensure the env override is unset / FS is empty.
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "cursor-cli-home-pin-final-"));
  try {
    withEnv(
      {
        HOME: home,
      ...
      () => {
        resetCursorAgentCliVersionCache();
        const a = getCursorAgentCliVersion();
        const b = getCursorAgentCliVersion();
        assert.equal(a, b, "deterministic with empty HOME");
        assert.equal(a, CURSOR_AGENT_CLI_VERSION, "falls back to pinned default");
        assert.match(a, /^\d{4}\.\d{2}\.\d{2}-[0-9a-f]+$/);
      }
    );
  } finally {
    resetCursorAgentCliVersionCache();
