...
test("TraeAuthModal.tsx no longer embeds the raw client_id literal (Hard Rule #11)", () => {
  const src = fs.readFileSync(
    path.join(repoRoot, "src/shared/components/TraeAuthModal.tsx"),
    "utf8"
  ) as string;
  assert.ok(
    !src.includes("en1oxy7wnw8j9n"),
    "TraeAuthModal.tsx must resolve the client_id via resolvePublicCred(), not a string literal"
  );
  assert.ok(
    src.includes('resolvePublicCred("trae_id")'),
    "TraeAuthModal.tsx must call resolvePublicCred for the Trae client_id"
  );
});
